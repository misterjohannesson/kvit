...
- Loading (`.loading`): a client-side navigation that takes longer than 150 ms shows a 2px `--accent-500` bar sliding along the top of the content and a see-through veil (`--bg-canvas` at 45 %) over the content, rail untouched. No spinners. Reduced motion: static full-width bar.
- Alert callout (`.callout--alert`): the one message that needs action today, e.g. an issued document that has not been marked as sent. Overdue ramp on a bordered box with a display-weight title and the action inline; never a page-wide tint, and the text always carries the meaning. The matching `Ikke sendt` marker (`.badge--usendt`) sits next to the status badge in lists.
...
- The wordmark prints as text (`Kvit`, `--font-display`), not as the brass mark: the invoice is a legal document, not a brand surface. In the app the issued invoice carries no Kvit branding at all: it is the owner's document to their customer, so only the sender identity appears (owner decision pending, see README).
