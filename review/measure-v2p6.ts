/**
 * Spec v2 — pass 6 probe (commit affe23c, finding 18) — measure-v2p5.ts re-run against the pass-6 instance. Reversible: moves the opening-balance date forward
 ...
 * Usage: npx tsx review/measure-v2p6.ts   (BASE_URL / APP_PASSWORD env override the defaults)
 */
...
const PROBE = `(() => { const lines = ${LINES}; const sec = [...document.querySelectorAll('section')].find((s) => s.querySelector('h2')?.textContent.trim() === 'Pr. måned');
  ...
  return { monoCount: monos.length, monos, pFont: pcs.fontFamily, pSize: pcs.fontSize, pLineHeight: pcs.lineHeight, pColor: pcs.color, numericVar, sansVar,
    bareTextNodes: bare.length, bareTexts: bare, spaceBeforeExcluded: /åbningssaldoen\\. \\d/.test(p.textContent), boundaryCharCode: p.textContent.charCodeAt(p.textContent.indexOf('åbningssaldoen.') + 15), krInMono: monos.filter((s) => /kr\\./.test(s.text)).map((s) => s.text), bareWithDigits, intro: p.textContent.trim(), introLines: lines(p), introW: Math.round(p.getBoundingClientRect().width), introH: Math.round(p.getBoundingClientRect().height), introMaxW: pcs.maxWidth,
...
(async () => {
  ...
  try {
    ...
    for (const w of [1440, 1280, 1152]) {
      ...
      console.log('B', w, 'monoCount', (B[String(w)] as any).monoCount, 'bareWithDigits', JSON.stringify((B[String(w)] as any).bareWithDigits), 'space', (B[String(w)] as any).spaceBeforeExcluded, 'krInMono', JSON.stringify((B[String(w)] as any).krInMono));
  ...
  // Phase B vs pass-5 phase B (same shifted date): only the intro text / the 7th mono text may differ.
  const p5Path = path.join(process.cwd(), 'review', 'measure-v2p5.json');
  if (fs.existsSync(p5Path)) {
    const p5 = JSON.parse(fs.readFileSync(p5Path, 'utf8'));
    const FIELDS_B = ['heads', 'cellSubs', 'wrap', 'table', 'hiddenPx', 'rowHeights', 'colWidths', 'introLines', 'introW', 'introH', 'introMaxW', 'kpiSub', 'kpiSubLines', 'kpiW', 'numCells', 'numNotRight', 'numFontsNotMono', 'otherTables', 'pageHScroll', 'rows', 'foot', 'intro', 'monoCount', 'monos', 'pFont', 'pSize', 'pLineHeight', 'bareTextNodes', 'bareWithDigits'];
    const diffB: Record<string, Record<string, { p5B: unknown; p6B: unknown }>> = {};
    for (const w of ['1440', '1280', '1152']) {
      diffB[w] = {};
      for (const f of FIELDS_B) {
        const a = JSON.stringify(p5.B?.[w]?.[f]); const b = JSON.stringify((m.B as any)?.[w]?.[f]);
        if (a !== b) diffB[w][f] = { p5B: p5.B?.[w]?.[f], p6B: (m.B as any)?.[w]?.[f] };
      }
    }
    m.diffBVsP5 = diffB;
    console.log('phase-B diff vs pass 5:', JSON.stringify(diffB, null, 1));
  }

  fs.writeFileSync(path.join(OUT, '..', 'measure-v2p6.json'), JSON.stringify(m, null, 2));
  await browser.close();
