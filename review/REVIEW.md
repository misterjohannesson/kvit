...
| Kontoplan expansion (groups, archiving, 29-account default, CSV download/upload, grouped Resultat) | spec, quality (API tests: round trip, atomic refusal, archived-account rules, last-active guard, prune) | fixed during build: absent `archived` in a partial update must stay absent (union with undefined instead of `.optional()` around the preprocess); vouchers cannot be deleted, so tests use seeded expenses | 0 |
