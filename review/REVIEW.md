...
| Kontoplan (groups, archiving, CSV download/upload, optional 29-account template, grouped Resultat) | spec, quality (API tests: round trip, atomic refusal, archived-account rules, last-active guard, prune, template) | fixed during build: absent `archived` in a partial update must stay absent (union with undefined instead of `.optional()` around the preprocess); vouchers cannot be deleted, so tests use seeded expenses; the owner kept the eleven-account default, so the larger plan became a template instead of a seed | 0 |
| Balance-account mapping + derived journal (`posteringer.csv`), export completed (customers, settings, attachments, audit actor), restore from the export zip with confirm step and `backups/data.bakNN/` copies | spec, quality (API tests: journal balances per postering, unchanged round trip keeps the books identical, accountant correction applied, eight refusal cases, page staging + discard) | guard triggers dropped and re-created inside the restore transaction, verified against `REQUIRED_TRIGGERS` before commit; files swapped only after the database committed | 0 |
| Suppliers as rows (dropdown + "new supplier" text, rename/delete), expense report (accounts × suppliers × months), Kontoudtog (every bank flow with primo/ultimo per date window, presets + custom range, CSV) | spec, quality (API tests: hand-computed Q2 report and April ledger from the seed, supplier reuse ignoring case, form select vs typed name, migration of the supplier text column on a populated database, restore of an export without suppliers.csv) | migration 0013 rebuilds `expense` with `supplier_id` and creates one supplier per distinct trimmed name; `drizzle-kit generate` needs a TTY for the rename question, so the snapshot was produced in two unambiguous steps and merged | 0 |
