...
const movementSchema = z.object({
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Dato skal være åååå-mm-dd'),
  description: z.string().trim().min(1, 'Beskrivelse er påkrævet').max(300),
  /** Signed øre: positive = money in, negative = money out. */
  amountOre: z.coerce.number().int('Beløb skal være hele øre').max(1e13).min(-1e13).refine((n) => n !== 0, 'Beløb skal være forskelligt fra 0'),
